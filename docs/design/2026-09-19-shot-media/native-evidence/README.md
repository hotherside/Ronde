...
Captured on 19 September 2026 from the uncommitted `codex/clubhouse-native-studio` working tree, based on `d9b405c`. These are native SwiftUI Simulator screenshots, not HTML concepts. The media is the repository's generated test pattern; there is no private golf footage or tracking-accuracy evidence here.

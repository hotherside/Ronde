# Archived reviewer state: 30 August 2026

Preserved on 7 September 2026 from commit `6ff89b0` before the studio redesign reconciliation. This is historical evidence and includes dated working-tree statements; [Current State](../CURRENT_STATE.md) is the current authority.
...
Ronde is in pre-release product hardening and reviewer validation. The core Watch round flow and its reliability work remain unchanged. The current iPhone/iPad working tree adds an Apple-only entry and a complete Home, Library, Profile and individual-review shell around the one-video MVP. Reviews persist in an account-scoped local archive; raw video and analysis stay on device, while the live Supabase schema accepts only private profile and lightweight library metadata. Timestamp-based acquire-and-track ball analysis may seed a perspective-aware estimate from impact through a marked apex to landing. One renderer timeline now reveals a smoothly tapered solid-purple ribbon in chronological order along one smoothed path; estimated launch and continuation ranges use subtly lower opacity and explicit labels while a sampling-derived lag keeps the causal head behind the ball through apex and continuation. A short source preserves model timing through apex and may compress only descent, with landing/carry withheld if apex plus lag cannot fit. The continuation sheds detector/model join residual down-range and caps lateral landing growth inside an observed-direction corridor. Supported fits show `Model carry` with `Estimate · uncalibrated` only at landing. Unsupported clips still fail closed with `Ball flight not tracked`. The Supabase Apple provider is enabled for native client ID `com.ronde`; the Apple Developer App ID capability and signed-device login remain unverified, so live native login is not yet proven. Long-session slicing and hands-free review foundations are dormant. The latest signed-iPhone TestFlight check produced a tracer on only one of five owner-selected videos, and that path was not visually accurate. Together with the rejected exact-source output and dense-search experiment, this makes the current tracker an experimental checkpoint rather than accepted tracer accuracy. Held-out footage accuracy, representative output review, physical-device performance and distribution remain open gates.
...
| iPhone/iPad reviewer | Implemented in current working tree | Home, Library and Profile use a Caddie-led shell with Flightline-style real metrics and charts. Photos/Files import uses a labelled Add video action and real local thumbnails where available. Individual review is video-first; manual Impact/Apex/Landing editing opens full-screen with draft-safe Cancel/Save. Account-scoped persistence, favourites, optional place/course, club and notes, traced-video export/share and fail-closed unsupported clips remain connected. |
...
| Automatic tracer quality | Experimental; TestFlight recall 1/5 | The tracker decodes sequentially by source presentation timestamp and samples near 30 Hz. A 2 ms timing tolerance prevents measured 30.087 fps footage from collapsing to half-rate without over-sampling 50 to 240 fps inputs. Competing alternate full-frame paths establish a launch before local-ROI tracking, early detector hand-offs are trimmed and cost instrumentation is reported. A separate fail-closed source-frame check may anchor the ribbon to a uniquely identifiable stationary ball at impact. Playback and export use one cumulative-distance timeline and one tapered solid-purple ribbon, but the signed-iPhone five-video check produced only one inaccurate tracer. Evaluating every accepted acquisition window was also rejected after it selected a wrong object on the labelled Landscape A regression. Representative positive/negative evidence and accepted visual accuracy are still missing. |
...
| Preview routing | Implemented in current working tree | `RONDE_PREVIEW_SCREEN` supports deterministic sign-in, Home, Library, Profile, media-detail, full-screen tracer-editor and quick-review routes. |
| Automated tests | Verified 30 August 2026 | `Ronde iOS AppTests` covers account-scoped archive isolation, honest dashboard metrics, impact clustering/cooldown, body motion, clip windows, local media, orientation, source-frame launch anchoring, evidence-anchored completion, manual provenance, explicit Range opt-in, capture stability, chronological full-flight reveal, causal source-time timing, real-shot gating and selector invariance at 25/30/50/60/120/240 fps. The final run executed 74 checks: 72 passed, two optional external probes skipped and zero failed. |
...
| iPhone visual review | Partially verified 30 August 2026 | Fresh iPhone 17 Pro iOS 26.5 passes covered Apple-only sign-in, Home, Library, Profile, the individual video-first review and the full-screen manual tracer editor. Native Liquid Glass navigation, labelled Add video actions and the dark editing dock are visible. A separate exact 4K source export received a nine-frame visual pass from launch through landing for the continuous-ribbon tracer. These remain layout and one-clip rendering evidence, not representative detector, landing/carry or live-auth evidence. |
| iPad visual review | Verified 30 August 2026 | Fresh iPad Pro 11-inch iOS 26.5 passes covered Home, split individual review and the full-screen manual tracer editor. The centred canvas, chart hierarchy, system glass navigation, labelled import, direct handles and adaptive non-colliding review header rendered without clipping. |
...
- The owner’s latest TestFlight check produced only one inaccurate tracer from five videos. Those exact five clips are not yet in the external matrix, so their misses cannot be attributed confidently to impact timing, detector recall, association or completion geometry. The field result nevertheless invalidates any current reliability claim.
...
- Ran the supplied Landscape A original through the exact decoder, packaged model and selector in a bounded two-tile diagnostic corridor, then through the source-frame launch detector, perspective completion and exporter. It selected the same 11 mid-air points at `0.417` confidence. The independent launch anchor remained around `(0.524, 0.752)`. With unchanged detector evidence, the causal completion placed the apex near `(0.515, 0.122)` and bounded landing near `(0.488, 0.494)`, rather than magnifying the small observed left drift to the former foreground-return `x ≈ 0.404`; it presented the existing `150–220 m` model carry. The 3,840 x 2,160 export contains 190 frames at 30 fps. This demonstrates source-time and bounded-corridor invariants only. The latest owner frame review still rejects the visual apex, path and landing placement as off, so it is not acceptance evidence for automatic flight, landing or carry accuracy.
...
- Completed a second iPhone 17 Pro and iPad Pro 11-inch iOS 26.5 visual pass after the immersive-review refinement. The full-screen tracer dock remained readable over a dark stage; the iPad review split kept Favourite and trace provenance in the information header rather than colliding with video chrome. These are still deterministic fixture/layout checks.
...
- Ran the focused selector and evidence-anchored suites after causal apex timing and down-range continuation bounding: 35 checks executed, 34 passed, one optional external-matrix check skipped and zero failed. Coverage includes unique-versus-ambiguous launch objects, exact anchor start, launch-observation-continuation ordering, unchanged apex time under short-source fitting, post-apex-only compression, causal continuation lag, source-too-short landing withholding, zero landing residual, bounded observed-direction displacement, Reduced Motion and playback/export keyframe equivalence. Result bundle: `/tmp/ronde-causal-tracer-derived/Logs/Test/Test-Ronde iOS-2026.08.30_19-53-42-+1000.xcresult`.
- Ran the final complete `Ronde iOS` suite with the continuous solid-purple ribbon on iPhone 17 Pro iOS 26.5 Simulator: 66 checks executed, 64 passed, two optional external probes skipped and zero failed. Result bundle: `/tmp/ronde-continuous-tracer-derived/Logs/Test/Test-Ronde iOS-2026.08.30_18-20-24-+1000.xcresult`.
- Ran the final complete `Ronde iOS` suite after source-frame launch anchoring, exact marker placement and short-source landing completion on iPhone 17 Pro iOS 26.0 Simulator: 71 checks executed, 69 passed, two optional external probes skipped and zero failed. Result bundle: `/Users/hojaejung/Library/Developer/Xcode/DerivedData/Ronde-gfhuzlyeekbdpyajufawjloxnklj/Logs/Test/Test-Ronde iOS-2026.08.30_18-47-34-+1000.xcresult`.
- Re-ran the complete `Ronde iOS` suite on iPhone 17 Pro iOS 26.0 Simulator before checkpoint delivery: 74 checks executed, 72 passed, two optional external-media probes skipped and zero failed. Result bundle: `/tmp/ronde-causal-tracer-derived/Logs/Test/Test-Ronde iOS-2026.08.30_20-02-10-+1000.xcresult`.
- Added a measured-30 cadence regression: 31 source timestamps at 30.087 fps remain eligible instead of dropping to approximately 15 Hz, while the existing 25 to 240 fps invariance test still passes. The focused selector suite passed on iPhone 17 Pro iOS 26.0 Simulator.
- Ran and rejected an every-window acquisition experiment. The exact full-frame Landscape A probe completed in 265.965 seconds on CPU-only Simulator inference but selected a launch at `y = 0.3903`, outside the labelled `0.20...0.30` mid-air corridor. The alternate-window acquisition policy remains in production.
...
2. Transfer and label the exact five TestFlight videos, reproduce each miss through the instrumented local pipeline, then expand to the documented 20 to 30 clip validation matrix. Report impact timing, acquisition recall, false-tracer rate, target association, point error and per-clip cost separately.
...
5. Start the licensed golf-specific detector lane now: secure consent for the failed clips, label the ball across visible frames, establish a reproducible training path and compare a Core ML golf detector against the current WASB baseline. Do not claim training support from the upstream WASB repository while its training instructions remain unavailable, and do not distribute AGPL Ultralytics code or weights without a separate licence.

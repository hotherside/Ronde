...
| Automatic tracer quality | Experimental; visual accuracy unresolved | The tracker decodes sequentially by source presentation timestamp, samples at a 30 Hz analysis cadence, lets competing full-frame paths establish a launch before local-ROI tracking, trims early detector hand-offs and reports cost instrumentation. A separate fail-closed source-frame check may anchor the ribbon to a uniquely identifiable stationary ball at impact. Playback and export use one cumulative-distance timeline and one tapered solid-purple ribbon: estimated launch reveals first at lower opacity, the fully opaque observed range stays behind the ball, and lower-opacity estimated continuation follows only after observation reaches its endpoint. Model timing is retained through apex with at most 50 ms causal lag; only descent may compress into a short source. Join residual decays to zero and lateral landing displacement remains bounded in the observed direction. A passing track may seed estimated launch, apex, landing and broad carry. The exact-source run exercises these invariants, but its latest owner review still rejects the resulting flight and landing placement as visually off. Representative positive/negative evidence and accepted visual accuracy are still missing. |

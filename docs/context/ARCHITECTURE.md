...
RondeCompanionApp
  ...
  -> AVPlayer video-first review canvas
       ...
       -> perspective-aware launch-to-landing fit anchored to observed points
       -> solid purple observed geometry plus dashed purple estimated geometry
...
- `EvidenceAnchoredFlightPathExtrapolator` accepts only a valid, rising, source-timed observed track. Physical flight time begins at the independently detected impact, not at the first mid-air observation. It grid-searches bounded launch distance, speed, elevation and horizon assumptions, solves vertical perspective scale and horizon from all observed samples, fits lateral perspective motion, retains the detector-attributed points and creates separate estimated geometry from impact to first observation and from the last observation through apex to landing. Normalised screen rate chooses only a broad chip-like, medium or full-shot launch prior; it is not converted into measured speed. Near-best selection scores produce a rounded 20th-to-80th-percentile carry range using a separate down-range damping assumption from the stronger screen-perspective convergence. The range is explicitly modelled and uncalibrated; precise carry and physical apex height are not supported.
- `TimedTrajectoryPath` validates one strictly increasing presentation timestamp per detector point, applies bounded Catmull-Rom interpolation only for rendering, and maps media time to cumulative path length. `PlayerSynchronizedTracer` reads `AVPlayer` item time and renders a small sample-derived gap between the solid observed trail and the current ball, so no future observation can lead or cover it. `TracedVideoExporter` uses the same timing-to-path keyframes. Dashed connector/continuation geometry, model carry and an estimated apex fade in only after the observed trail completes. Reduced Motion removes progressive motion while preserving provenance.

...
- Ship the reviewed library/studio source change, then run a signed-iPhone matrix for import, analysis, pause/resume, frame stepping, trim, all four output canvases, audio, cancellation and native sharing. Include slow-motion/VFR, rotated portrait, no-audio and unsupported input.
...
- Add batch import and range-session cutting only after the single-shot editor earns usability and performance acceptance.
...
- Consider long-session segmentation, hands-free capture and automatic replay after short-shot reliability is established.

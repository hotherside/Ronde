...
- After the owner reported tracking stuck at 10% followed by a crash, localised that state to the first inference after model loading. No crash/jetsam report was available and the phone was disconnected, so the cause remains unconfirmed. Released the full-source RGB cache before inference, added cancellation checks around loads/predictions and selected CPU/Neural Engine for physical iOS. A real-phone rerun remains necessary; these changes are containment, not verified crash resolution.
...
- App/domain/media tests pass: 141 passed, two optional skips, zero failures on iPhone 17/iOS 27 Simulator (`ronde-feedback-units-v3.xcresult`). Two initial compile failures were fixed before this run. UI and signed-device results are recorded in Current State as they complete. Earlier assisted development scores do not validate the new automatic-seed flow or physical compute configuration.
- Added [ADR 0018](decisions/0018-clip-first-tracing-and-correction.md). Source delivery remains pending on `codex/tracer-evidence-benchmark`; broader design work is preserved separately.

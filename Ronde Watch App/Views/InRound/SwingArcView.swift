...
/// Background track shows the full range; foreground fills to `normalizedForce`.
...
struct SwingArcView: View {
    ...
    /// Arc fill: normalizedForce mapped to 75% of a full circle (270°).
    private var trimEnd: CGFloat {
        CGFloat(normalizedForce) * 0.75
    }
    ...
    private var arcGradient: AngularGradient {
        AngularGradient(
            ...
            endAngle: .degrees(135 + 270 * normalizedForce)
        )
    }
    ...
    var body: some View {
        ZStack {
            ...
            // Foreground arc — fills to normalizedForce
            Circle()
...
#Preview("Arc Forces") {
    VStack(spacing: 16) {
        HStack(spacing: 12) {
            SwingArcView(normalizedForce: 0.2, diameter: 70)
            SwingArcView(normalizedForce: 0.5, diameter: 70)
        }
        HStack(spacing: 12) {
            SwingArcView(normalizedForce: 0.8, diameter: 70)
            SwingArcView(normalizedForce: 1.0, diameter: 70)
        }
    }
    .padding()
    .background(.black)
